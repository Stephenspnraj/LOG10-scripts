...
pincode_id_ip = int(os.environ['pincode_id'])
loc_type_ip = 'FM'
...
def main():
    ...
    print(f"({pincode_id_ip},{old_loc_ip},{new_loc_ip},'{loc_type_ip}','{is_dry_run}');")
    
    if old_loc_ip and new_loc_ip and pincode_id_ip and loc_type_ip and is_dry_run is not None:
        insert_config_query = (f"INSERT INTO path_correction_config(pincode_id,old_loc,new_loc,loc_type,is_dry_run) VALUES "
                               f"({pincode_id_ip},{old_loc_ip},{new_loc_ip},'{loc_type_ip}',{is_dry_run});")
        try:
    ...
    for index, row in df.iterrows():
        ...
        print(f"{row_id} - Started from pincode {pincode_id} from {old_loc} to {new_loc}")
        ...
        main_query = f"""select cep.id as id, cep.waybill_no as waybill_no
                        ...
                        where shipper_pincode_id = {pincode_id}
        ...
        print(f"Done for Pincode ID : {pincode_id}")
