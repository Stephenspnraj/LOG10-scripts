...
def existing_location_onboarding_api(token, tokenId, partner_id, dccode, client_location_name, dc_address, loc_zipcode, delivery_pincodes, city_id, sc):
    ...
    delivery_pincodes_list = [int(pincode.strip()) for pincode in delivery_pincodes.split(',') if pincode.strip()]
    payload = {
        "partner": {
            "id": partner_id
        },
...
def metadata_insert(dccode, sc, conn):
    try:
        with conn.cursor() as cursor:
            location_query = "SELECT id FROM locations WHERE (alias = %s OR alias = %s)"
            ...
            if location_result and len(location_result) == 2:
                ...
                check_query = (
                    "SELECT COUNT(*) as cnt FROM loadshare.network_metadata WHERE is_active=1 "
                    "AND location_alias = %s AND next_location_alias = %s"
                )
                metadata_exists = execute_query(check_query, conn, (location_alias, next_location_alias))
                count = metadata_exists[0]['cnt']
                if count == 0:
                    insert_query = (
                        "INSERT INTO loadshare.network_metadata "
                        "(location_alias, next_location_alias, is_active) VALUES(%s, %s, 1)"
                    )
...
def route_creation(dccode, sc, conn):
    try:
        with conn.cursor() as cursor:
            ...
            location_result = execute_query(location_query, conn, (dccode, sc))
            if location_result and len(location_result) == 2:
                source_query = "SELECT entity_id AS sourcePartnerId, id AS sourceLocationId FROM locations WHERE alias = %s"
                destination_query = "SELECT id AS destinationLocationId FROM locations WHERE alias = %s"
                ...
                if source_result and destination_result:
                    ...
                    try:
                        ...
                        logging.info("Route: %s Response code: %s, Message: %s", route_name, status_code, message)
                    except Exception:
                        logging.warning("Failed to parse route creation response.")
                else:
                    logging.info("No matching record found in the database for source: %s or destination: %s", dccode, sc)
            else:
...
def update_wrong_facility_scan(dccode, conn):
    try:
        with conn.cursor() as cursor:
            location_id_query = "SELECT id AS location_id FROM locations WHERE alias = %s"
            ...
            if location_id_result:
                ...
                is_present_query = "SELECT JSON_CONTAINS(config->'$.enableWrongFacilityShipmentScan', %s) AS is_present FROM application_config WHERE id = 1"
                ...
                if not list(is_present_result[0].values())[0]:
                    ...
                    conn.commit()
                    logging.info("%sWrongFacility updated for location: %s => %s%s", Fore.GREEN, dccode, location_id, Style.RESET_ALL)
                else:
                    logging.info("Location: %s already present in enableWrongFacilityShipmentScan", dccode)
            else:
...
def update_call_masking_exclude_location(dccode, conn):
    try:
        with conn.cursor() as cursor:
            ...
            if location_id_result:
                location_id = location_id_result[0]['location_id']
                is_present_query = "SELECT JSON_CONTAINS(config->'$.call_masking_exclude_location', %s) AS is_present FROM application_config WHERE id = 1"
                is_present_result = execute_query(is_present_query, conn, (str(location_id),))
                if not list(is_present_result[0].values())[0]:
                    update_query = """
                    UPDATE application_config
                    ...
                    cursor.execute(update_query, (location_id,))
                    conn.commit()
                    logging.info("%sCall masking exclusion updated for location: %s => %s%s", Fore.GREEN, dccode, location_id, Style.RESET_ALL)
                else:
                    logging.info("Location: %s already present in call_masking_exclude_location", dccode)
            else:
...
def process_input_and_configure_system():
    ...
    input_file = 'old_location_v2.csv'
    output_file = 'old_location_output_v2.csv'
    success_count = 0
    fail_count = 0
    ...
    with pymysql.connect(**log10_conf) as conn, \
        ...
        for i, row in enumerate(csv_reader, start=1):
            ...
            exists_query = "SELECT * FROM locations WHERE client_location_name=%s AND status = 1"
            ...
            if branch_admin_name and contact_number and email_id:
                response = existing_location_onboarding_api_bu(
                    ...
                    dccode, client_location_name, dc_address, loc_zipcode, delivery_pincodes, city_id, sc
                )
