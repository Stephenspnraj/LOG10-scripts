...
def partner_location_api(token, tokenId, partner_name, contact_number, branch_admin_name,
                        email_id, fmcode, client_location_name, fmcodeaddress,
                        loc_zipcode, pickup_pincodes, city_id, fmsc, is_loadshare_partner,is_migrated_location):
    headers = {
        'token': token,
        'tokenId': tokenId,
        'Content-Type': 'application/json'
    }
    pickup_pincodes_list = [int(p.strip()) for p in pickup_pincodes.split(',') if p.strip()]
    payload = {
        "partner": {
            "name": partner_name,
        ...
        with pymysql.connect(**mysql_conf) as conn, \
            ...
            for i, row in enumerate(csv_reader, start=1):
                ...
                response = partner_location_api(
                    ...
                    fmcode, client_location_name, fmcodeaddress, loc_zipcode, pickup_pincodes, city_id,
                    fmsc, is_loadshare_partner,is_migrated_location
                )
