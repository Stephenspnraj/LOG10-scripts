...
#     if not existing:
#         logger.info("➡ No active mapping to deactivate.")
#         return 0
#     base_source = base(source)
#     base_dest   = base(destination)
#     dest_suffix = destination.split(".")[-1] if "." in destination else "FMSC"
...
#                SET is_active = 0,
#                    audit_log='Deactivated by National Route Script'
#              WHERE (location_alias = %s OR location_alias = %s)
#                AND (next_location_alias = %s OR next_location_alias = %s)
#                AND is_active = 1
#         """, (source, base_source, f"{base_dest}.{dest_suffix}", base_dest))
...
def deactivate_mapping(conn, source, destination):
    ...
    # 🔹 Show mappings before deactivation (restored)
    existing = show_existing_network_metadata(conn, source, destination)
    if not existing:
        logger.info("➡ No active mapping to deactivate.")
        return 0
...
#     base_dest   = base(destination)
#     dest_suffix = destination.split(".")[-1] if "." in destination else "FMSC"
#     base_cross  = base(crossdock) if crossdock else None
#     cross_suffix = crossdock.split(".")[-1] if crossdock and "." in crossdock else "FMSC"
...
#             """, (source, base_source,
#                   f"{base_dest}.{dest_suffix}", base_dest,
#                   f"{base_cross}.{cross_suffix}", base_cross))
#         else:
#             cur.execute("""
#                 SELECT id FROM network_metadata
#                  WHERE (location_alias = %s OR location_alias = %s)
...
#                 UPDATE network_metadata
#                 SET is_active = 1,
#                     audit_log = 'Activated by National Route Script'
#                 WHERE id = %s
#             """, (existing["id"],))
#             action = "Activated"
#         else:
...
#                     VALUES (%s, %s, %s, 1, 'Activated by National Route Script')
#                 """, (source, f"{base_dest}.{dest_suffix}", f"{base_cross}.{cross_suffix}"))
#             else:
#                 cur.execute("""
#                     INSERT INTO network_metadata (location_alias, next_location_alias, is_active, audit_log)
#                     VALUES (%s, %s, 1, 'Activated by National Route Script')
#                 """, (source, f"{base_dest}.{dest_suffix}"))
#             action = "Inserted"
#     conn.commit()
...
def activate_or_insert_mapping(conn, source, destination, crossdock=None):
    logger.info(f"\n--- Checking EXISTING mappings before activation for {source} → {destination} ---")
    show_existing_network_metadata(conn, source, destination)
    ...
    with conn.cursor() as cur:
        if crossdock:
            cur.execute("""
                SELECT id, is_active FROM network_metadata
                WHERE SUBSTRING_INDEX(location_alias, '.', 1) = %s
                  AND SUBSTRING_INDEX(next_location_alias, '.', 1) = %s
                  AND SUBSTRING_INDEX(crossdock_alias, '.', 1) = %s
                LIMIT 1
            """, (base_source, base_dest, base_cross))
        else:
            cur.execute("""
                SELECT id, is_active FROM network_metadata
                WHERE SUBSTRING_INDEX(location_alias, '.', 1) = %s
        ...
        if existing:
            cur.execute("""
                UPDATE network_metadata
                SET is_active = 1,
                    audit_log = CONCAT('Re-activated by National Route Script: ', %s, ' → ', %s),
                    updated_at = NOW()
                WHERE id = %s
            """, (source, destination, existing["id"]))
            action = "Re-activated"
        else:
            if crossdock:
                cur.execute("""
...
def show_pending_manifests(conn, current_alias, destination_alias, next_alias=None):
    with conn.cursor() as cur:
        ...
        """, (base(current_alias), base(destination_alias)))
        rows = cur.fetchall()
...
def main():
    ...
    for i, row in df.iterrows():
        ...
        if not FMCD and not LMCD:  # Case 1
            actions.append(f"Deactivated({deactivate_mapping(conn, location_alias, next_location_alias)})")
            actions.append(activate_or_insert_mapping(conn, location_alias, next_location_alias, None))
            if needs_correction:
            ...
            actions.append(f"Deactivated({deactivate_mapping(conn, location_alias, next_location_alias)})")
            actions.append(activate_or_insert_mapping(conn, location_alias, next_location_alias, crossdock_alias))
            if needs_correction:
            ...
            leg1 = [f"Deactivated({deactivate_mapping(conn, alias1, next_location_alias)})",
                    activate_or_insert_mapping(conn, alias1, next_location_alias, cross1)]
            leg2 = [f"Deactivated({deactivate_mapping(conn, alias2, next_location_alias)})",
                    activate_or_insert_mapping(conn, alias2, next_location_alias, cross2)]
            actions.append(f"Leg1: {', '.join(leg1)} | Leg2: {', '.join(leg2)}")
            if needs_correction:
