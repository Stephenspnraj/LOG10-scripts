...
def partner_location_api(token, tokenId, partner_name, contact_number, branch_admin_name,
                        sc_code, client_location_name, sc_address,
                        loc_pincode, city_id, is_loadshare_partner, gst_number):
    headers = {
        'token': token,
        'tokenId': tokenId,
...
def process_input_and_configure_system():
    ...
    input_file = 'new_partner_sc_onboarding_input.csv'
    output_file = 'new_partner_sc_onboarding_output.csv'
    success_count = 0
    fail_count = 0
    ...
    with pymysql.connect(**mysql_conf) as conn, \
        ...
        for i, row in enumerate(csv_reader, start=1):
            ...
            response = partner_location_api(
                token, tokenId, partner_name, contact_number, branch_admin_name,
                sc_code, client_location_name, sc_address, loc_pincode, city_id,
                is_loadshare_partner, gst_number
            )
